...
final class MessagesServiceProvider: NSObject, AppAutomationServiceProvider {
    var appName: String { "Messages" }
    ...
    func perform(action: AutomationAction) -> AutomationResponse<AnyCodableValue> {
        ...
        guard let (appElement, pid) = axDriver.connect(appName: appName) else {
            return AutomationResponse(success: false, errorCode: .notConnected, message: "Messages not running")
        }
    ...
    private func handleInvoke(action: AutomationAction) -> AutomationResponse<AnyCodableValue> {
        ...
        if command == "send" {
            guard let recipient = action.params?["to"]?.value as? String,
                  let body = action.params?["body"]?.value as? String else {
                return AutomationResponse(success: false, errorCode: .invalidRequest, message: "Missing to/body")
            }
            let script = "tell application \"Messages\" to send \"\(escapeForAppleScript(body))\" to buddy \"\(escapeForAppleScript(recipient))\""
            return runAppleScript(script)
        }
    ...
    private func sendShortcut(action: AutomationAction) -> AutomationResponse<AnyCodableValue> {
        ...
        if keyString.lowercased().contains("new_message") {
    ...
    private func runAppleScript(_ script: String) -> AutomationResponse<AnyCodableValue> {
        ...
        return AutomationResponse(success: true, message: "OK", data: AnyCodableValue(response))
    }
...
final class MessagesXPCDelegate: NSObject, NSXPCListenerDelegate {
    func listener(_ listener: NSXPCListener, shouldAcceptNewConnection newConnection: NSXPCConnection) -> Bool {
        newConnection.exportedInterface = NSXPCInterface(with: AppAutomationXPCProtocol.self)
        newConnection.exportedObject = AutomationXPCService(provider: MessagesServiceProvider())
...
let delegate = MessagesXPCDelegate()
